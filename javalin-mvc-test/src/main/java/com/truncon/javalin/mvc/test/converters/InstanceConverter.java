...
public final class InstanceConverter {
    ...
    @Converter("instance-model-converter")
    public ConversionModel convert(HttpContext context, String name, ValueSource valueSource) {
        ...
        model.setByte(parseByte(getString(lookup, "byte")));
        model.setChar(parseChar(getString(lookup, "char")));
        model.setDouble(parseDouble(getString(lookup, "double")));
        model.setFloat(parseFloat(getString(lookup, "float")));
        model.setInteger(parseInt(getString(lookup, "int")));
        model.setLong(parseLong(getString(lookup, "long")));
        model.setShort(parseShort(getString(lookup, "short")));
        return model;
    }
